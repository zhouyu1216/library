...
Book::Book()
    : title("Unknown"), isbn("0000000000"), publisher("Unknown"),
...
Book::Book(const string& title, const string& isbn, const string& publisher,
    ...
    if (isValidIsbn(isbn)) {
        ...
        cout << "[Warning] Invalid ISBN, using default." << endl;
    }
}
...
void Book::setPrice(double price) {
    ...
    else cout << "[Warning] Price cannot be negative." << endl;
}

void Book::setPages(int pages) {
    if (pages >= 0) this->pages = pages;
    else cout << "[Warning] Pages cannot be negative." << endl;
}
...
bool Book::setIsbn(const string& isbn) {
    ...
    cout << "[Warning] Invalid ISBN, update failed." << endl;
    return false;
}
